...
// Test Registry loaded from tests.json (generated by build_workspace.py)
let TEST_REGISTRY = {};
...
function selectFile(filename) {
  ...
  updateLineNumbers();
  updateFileList();
  // Show/hide Test button based on whether this file has tests
  if (TEST_REGISTRY[filename] && !TEST_REGISTRY[filename].skip) {
    els.btnTest.classList.remove('hidden');
  } else if (!Object.keys(TEST_REGISTRY).length) {
    // Registry not loaded yet, keep button visible for exercise files
    els.btnTest.classList.remove('hidden');
  } else {
    els.btnTest.classList.add('hidden');
  }
}
...
els.btnTest.onclick = runDiagnosticTests;
...
function runDiagnosticTests() {
  ...
  // Look up test from registry
  const testEntry = TEST_REGISTRY[STATE.activeFile];
  let testSuite;
  let testMethod = null;

  if (testEntry && !testEntry.skip) {
    testSuite = testEntry.testCode;
    testMethod = testEntry.testMethod;
  } else {
    // Fallback: generic compilation/import check
...
class CompilationTest(unittest.TestCase):
    def test_import(self):
        import importlib
        mod = importlib.import_module("${cleanModName}")
        self.assertIsNotNone(mod, "Module loaded successfully")
`;
  }
...
function handleTestResult(status, message) {
  ...
  // Reset outcome title classes
  els.testOutcomeTitle.className = 'test-outcome-title';

  // Try parsing detailed per-method results
  let methods = [];
  try {
    methods = JSON.parse(message);
  } catch (e) {
    // Not JSON — fallback to simple display
  }

  const summaryBar = document.getElementById('test-summary-bar');
  const methodsList = document.getElementById('test-methods-list');

  if (Array.isArray(methods) && methods.length > 0) {
    // Render summary bar
    const passed = methods.filter(m => m.status === 'PASS').length;
    const failed = methods.filter(m => m.status === 'FAIL').length;
    const errors = methods.filter(m => m.status === 'ERROR').length;
    const skipped = methods.filter(m => m.status === 'SKIP').length;
    const total = methods.length;

    let summaryParts = [`${passed}/${total} passed`];
    ...
    // Render per-method list
    methodsList.innerHTML = '';
    methods.forEach(m => {
      const row = document.createElement('div');
      const statusClass = m.status.toLowerCase();
      row.className = `test-method-row test-${statusClass}`;

      const icons = { PASS: '\u2705', FAIL: '\u274c', ERROR: '\u26a0\ufe0f', SKIP: '\u23ed\ufe0f' };
      const icon = icons[m.status] || '\u2753';

      const label = document.createElement('span');
      label.textContent = `${icon} ${m.method}`;
      row.appendChild(label);

      if (m.message && m.status !== 'PASS') {
        const detail = document.createElement('div');
        detail.className = 'test-method-detail';
    ...
    // Set overall outcome
    els.testOutcomeDetails.textContent = '';
    if (status === 'PASS') {
      els.testOutcomeTitle.classList.add('pass');
      els.testOutcomeTitle.innerText = 'ALL TESTS PASSED \u2705';
    } else {
      els.testOutcomeTitle.classList.add('fail');
      els.testOutcomeTitle.innerText = 'SOME TESTS FAILED';
    }
  } else {
    // Fallback: simple message display (no per-method breakdown)
...
window.onkeydown = (e) => {
  ...
  // Escape -> Close test results dialog
  if (e.key === 'Escape') {
    els.dialogTestResults.classList.add('hidden');
  }
...
window.onload = () => {
  const loadWorkspace = fetch('workspace.json')
    .then(res => {
  ...
  const loadTests = fetch('tests.json')
    .then(res => {
      if (!res.ok) throw new Error("tests.json not found");
      return res.json();
    })
    .then(data => {
      TEST_REGISTRY = data;
      console.log(`Loaded test registry: ${Object.keys(data).length} entries`);
    })
    .catch(err => {
      console.warn("Could not load test registry:", err);
    });

  Promise.all([loadWorkspace, loadTests]).finally(() => {
    updateFileList();
    updateLineNumbers();
